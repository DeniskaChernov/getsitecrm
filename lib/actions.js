...
const { calcMarginPct } = require('./money');

/** Canonical lead statuses that the UI funnel can group */
function normalizeLeadStatus(status) {
  const raw = str(status).trim();
  if (!raw) return '';
  const s = raw.toLowerCase();
  if (s.includes('нов')) return 'новая';
  if (s.includes('квалиф')) return 'квалификация';
  if (s.includes('работ')) return 'в работе';
  if (s.includes('кп') || s.includes('расч')) return 'КП';
  if (s.includes('переговор') || s.includes('решен')) return 'переговоры';
  if (s.includes('предоплат')) return 'предоплата';
  if (s.includes('отказ') || s.includes('проиг')) return 'отказ';
  return raw;
}
  ...
  switch (action) {
    ...
    case 'lead.create': {
      const lead = {
        ...
        clientName: str(body.clientName || body.name),
        companyName: str(body.companyName || body.company || body.clientName || body.name),
        phone: str(body.phone),
        source: str(body.source, 'Другое'),
        service: str(body.service),
        budget: num(body.budget),
        status: normalizeLeadStatus(body.status) || 'новая',
        temperature: 'тёплый',
    ...
    case 'lead.update': {
      ...
      Object.assign(lead, {
        clientName: str(body.clientName || body.name, lead.clientName),
        companyName: str(body.companyName || body.company, lead.companyName),
        ...
        status: body.status !== undefined ? normalizeLeadStatus(body.status) : lead.status,
    ...
    case 'lead.status': {
      const lead = data.leads.find((l) => l.id === body.id);
      if (!lead) throw Object.assign(new Error('Заявка не найдена'), { status: 404 });
      lead.status = normalizeLeadStatus(body.status) || lead.status;
    ...
    case 'service.update': {
      ...
      const nextWorking =
        body.workingPrice !== undefined
          ? num(body.workingPrice, service.workingPrice)
          : body.price !== undefined
            ? num(body.price, service.workingPrice)
            : service.workingPrice;
      const nextFrom =
        body.priceFrom !== undefined ? num(body.priceFrom, service.priceFrom) : service.priceFrom;
      const nextPremium =
        body.premiumPrice !== undefined
          ? num(body.premiumPrice, service.premiumPrice)
          : service.premiumPrice;
      if (isSales) {
        Object.assign(service, {
          priceFrom: nextFrom,
          workingPrice: nextWorking,
          premiumPrice: nextPremium,
    ...
    case 'estimate.create': {
      const lines = Array.isArray(body.lines)
        ...
        : Array.isArray(body.items)
          ? body.items
          : [];
      const clientFromId = body.clientId
        ? (data.clients || []).find((c) => c.id === body.clientId)
        : null;
      const amount = lines.reduce(
        (sum, line) => sum + num(line.qty || line.quantity, 1) * num(line.price || line.workingPrice),
        0
      );
      ...
      const estimatedCost = lines.reduce(
        (sum, line) => sum + num(line.qty || line.quantity, 1) * num(line.cost || line.costFloor || 0),
        0
      );
      const margin = calcMarginPct(discounted || amount, estimatedCost, 2);
      const estimate = {
        id: id(),
        number: `СМ-${191 + data.estimates.length}`,
        clientName: str(body.clientName, clientFromId?.name || ''),
        projectName: str(body.projectName || body.title),
    ...
    case 'payment.create': {
      const project = data.projects.find((p) => p.id === str(body.projectId));
      if (!project) throw Object.assign(new Error('Выберите существующий проект'), { status: 400 });
      const payment = {
        id: id(),
        projectId: project.id,
    ...
    case 'user.create': {
      ...
      const nameCheck = validateDisplayName(body.displayName || body.fullName);
      const systemRole = normalizeRole(body.systemRole || 'sales_manager') || 'sales_manager';
      if (!email || !body.password || !(body.displayName || body.fullName)) {
        throw Object.assign(new Error('Нужны имя, email и пароль'), { status: 400 });
      }
      if (!passwordCheck.ok) throw Object.assign(new Error(passwordCheck.error), { status: 400 });
      if (!nameCheck.ok) throw Object.assign(new Error(nameCheck.error), { status: 400 });
      if (!ROLES[systemRole]) {
        throw Object.assign(new Error('Неизвестная роль'), { status: 400 });
      }
    ...
    case 'user.activate': {
      if (!actor || actor.systemRole !== 'founder') {
        throw Object.assign(new Error('Только основатель может включать пользователей'), { status: 403 });
      }
      const email = str(body.email).trim().toLowerCase();
      if (!email) throw Object.assign(new Error('Укажите email'), { status: 400 });
      const user = (data.users || []).find((u) => u.email.toLowerCase() === email);
      if (!user) throw Object.assign(new Error('Пользователь не найден'), { status: 404 });
      user.active = true;
      auditLog('Включён пользователь', 'User', user.email);
      result = { ok: true, user: publicUser(user), users: data.users.map(publicUser) };
      break;
    }
    case 'user.password': {
      ...
      const passwordCheck = validatePassword(body.password || body.newPassword);
      if (!passwordCheck.ok) throw Object.assign(new Error(passwordCheck.error), { status: 400 });
      ...
      if (isSelf && !body.currentPassword) {
        throw Object.assign(new Error('Укажите текущий пароль'), { status: 400 });
      }
      if (isSelf && !verifyPassword(body.currentPassword, actor.passwordHash)) {
        throw Object.assign(new Error('Текущий пароль неверный'), { status: 401 });
      }
