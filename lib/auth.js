...
/** Revoked session ids until their natural expiry (logout / force-logout) */
const revokedSessions = new Map();

function pruneRevokedSessions() {
  const now = Date.now();
  for (const [jti, exp] of revokedSessions) {
    if (!exp || now > Number(exp)) revokedSessions.delete(jti);
  }
}
...
function getSessionUser(req) {
  ...
  const data = readSession(cookies[COOKIE_NAME]);
  if (!data) return null;
  if (data.jti && revokedSessions.has(data.jti)) return null;
  return data;
}
...
function revokeSessionFromRequest(req) {
  ...
  const data = readSession(cookies[COOKIE_NAME]);
  if (data?.jti) {
    revokedSessions.set(data.jti, data.exp || Date.now() + SESSION_TTL_MS);
  }
}
...
/**
 * Bootstrap-аккаунты. Email/пароль берутся из env (Railway Variables),
 * иначе — локальные дефолты для разработки.
 * AUTH_*_PASSWORD в production: если задан — применяется при каждом старте.
 */
